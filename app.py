...
def diff_pct(cur, prev):
    ...
    if prev == 0:
        return diff, "#DIV/0!"
    return diff, f"{diff / prev:.0%}"  # ✅ already no decimals
...
# =============================================================================
# DETAIL SALES & TRAFFIC (Excel) — for Listing Metrics
# =============================================================================
...
def compute_listing_metrics_tables() -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for client in CLIENTS:
        ...
        cur_agg = cur.groupby("asin", as_index=False).agg(
            ...
            cur_bb_x_pv=("featured_offer_pct", lambda s: float(pd.to_numeric(s, errors="coerce").fillna(0.0).values @ cur.loc[s.index, "pageviews_total"].astype(float).values) if len(s.index) else 0.0),
            cur_pv_for_bb=("featured_offer_pct", lambda s: float(cur.loc[s.index, "pageviews_total"].astype(float).values.sum()) if len(s.index) else 0.0),
        )
        ...
        merged["cur_buybox_pct"] = merged.apply(
            lambda r: (r["cur_bb_x_pv"] / r["cur_pv_for_bb"]) if r["cur_pv_for_bb"] and r["cur_pv_for_bb"] > 0 else None,
            axis=1
        )
...
def render_client_pages():
    ...
    with c1:
        ...
        st.caption(f"Last year: {fmt_currency(prev)} | Diff %: {fmt_pct0(pct_from_report if pct_from_report else fallback_pct)}")
    ...
    with c2:
        ...
        st.caption(f"Last month: {fmt_currency(prev)} | Diff %: {fmt_pct0(pct_from_report if pct_from_report else fallback_pct)}")
